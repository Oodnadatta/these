...
%FIXME : pourquoi mets-tu ta clé ssh publique dedans ?
%FIXME : elle apparaît avec des espaces avant les ":" dans le pdf final.
\vfill
...
%FIXME Penser à faire la section
...
Depuis Louis Pasteur, les bactéries ont toujours été associées aux maladies. La mise en évidence des agents pathogènes comme la syphilis ou la peste n'a pas aidé à tordre le cou de ce stéréotype. La médecine les a donc longtemps considérées comme les ennemies à combattre plutôt que des alliées. %FIXME Louis Pasteur, sinon petit moment de doute avec l'Institut Pasteur
...
\label{hyigienisme}
\end{figure}
%FIXME L'image semble étirée verticalement.
...
En microbiologie, un OTU est défini par un groupe de bactéries ayant une similarité dans leurs séquences d'ADNr 16S supérieur ou égal à 97 \%. %FIXME Ce serait bien de mettre une référence pour ces chiffres.
...
\paragraph{La dominance} caractérise un OTU dont l'abondance est supérieure à 90 \% dans un échantillon. %FIXME Ce serait bien de mettre une référence pour ces chiffres.
...
\end{mycapequ}
%FIXME : le 1 de CHAO1 devrait aussi être en italique car il fait partie du nom de l'indice et n'est pas un nombre indépendant.
...
Pour générer cette courbe, des groupes de reads de taille croissante (1..n) sont tirés aléatoirement depuis l'échantillon. Le groupe est reporté sur l'axe X et le nombre d'OTUs trouvé dans ce groupe est reporté sur l’axe Y. %FIXME On ne retrouvre que ce que l'on a perdu.
...
\end{figure}
%FIXME dans l'image : OTU avec un "s", nombre avec une majuscule, espace avant les ":" (x2), horizontale avec un e, raréfaction avec un accent aigu sur le e.
...
%FIXME le NGS n'est pas l'appareil, mais les techniques de séquençages ou le séquençage lui-même.
...
%FIXME : la métagénomique ne concerne pas que l'ADN, mais plus globalement le matériel génétique.
...
%FIXME Ces fichiers ne sont pas nécessairement produits par NGS (exemple Genebank)
...
\paragraph{Un graphe} est un objet mathématique défini par des nœuds (nodes) reliés par des arrêtes (edges). Les graphes permettent de répondre à différents problèmes mathématiques, comme par exemple le problème des sept ponts de Königsberg, qui consiste à savoir s'il existe une promenade passant par 7 ponts une seul fois. La théorie des graphes est très utilisée en bio-informatique.
%FIXME : sinon, c'est une phrase nominale
...
La loi de Hardy Weinberg estime qu’en Bretagne, 1 personne sur 27 est porteuse d'une mutation dans /textit{CFTR} à l’état hétérozygote.
%FIXME il n'y a pas qu'une seule mutation. On n'est pas nécéssairement patient lorsque l'on est porteur : le plus souvent, les personnes ignorent leur statut d'hétérozygote.
Cette haute prévalence s’explique probablement par un effet fondateur associé à un avantage sélectif pour les individus porteurs de l’allèle muté\footnote{Plusieurs hypothèses ont été proposées, notamment lors des grandes épidémies de choléra en diminuant les pertes hydriques.}.
...
On dénombre à ce jour 2017 mutations \cite{cftrdb} mises en cause dans la mucoviscidose.
%FIXME La base de données que tu utilises ne semble pas avoir été mise à jour depuis 2011.
La perte d’une phénylalanine en position 508 par délétion du triplet c.1521-1523delCTT (anciennement $\Delta$F508) cause à elle seule 80 \% des mucoviscidoses\cite{cftrdb}.
...
%FIXME Le test de Guthrie est la recherche de la phénylcétonurie. Par abus de langage, il est utilisé pour l'ensemble des tests néonataux, mais c'est une erreur.
%FIXME Test *à* la sueur est une erreur (encore un abus de langage).
